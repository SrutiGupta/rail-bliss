...
export function BookPage() {
  ...
  const trainQuery = useQuery({
    ...
    enabled: !!trainId,
    queryFn: () => http.get<Train>(`/trains/${trainId}`),
  });
  ...
  const availability = useQuery({
    ...
    queryFn: () =>
      http.get<{ available: number }>(
        `/trains/${trainId}/availability?date=${encodeURIComponent(journeyDate)}`,
      ),
  });
  ...
  const booking = useMutation({
    mutationFn: async () => {
      ...
      if ((availability.data?.available ?? 0) < passengers.length)
        throw new Error("Not enough seats available on this date");

      const data = await http.post<{ pnr: string }>("/bookings", {
        trainId,
        journeyDate,
    ...
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      <section className="mt-6 rounded-xl border border-border bg-card p-5 shadow-ticket">
        <div className="flex flex-wrap items-center gap-3">
          <span className="font-mono text-sm text-rail">{train.trainNumber}</span>
          <h2 className="text-lg">{train.trainName}</h2>
          <span className="rounded bg-secondary px-2 py-0.5 text-xs">{train.coachClass}</span>
        </div>
        <p className="mt-2 text-sm text-muted-foreground">
          {train.source} {formatTime(train.departureTime)} → {train.destination}{" "}
          {formatTime(train.arrivalTime)} · {formatMoney(train.fare)} per seat
        </p>
      </section>
          ...
          <div className="grid gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              {availability.isLoading
                ...
                : `${availability.data?.available ?? 0} seats available on ${formatDate(journeyDate)}`}
            </p>
          </div>
          <div className="grid gap-2">
